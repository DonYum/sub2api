...
const exportToExcel = async () => {
  ...
  try {
    ...
    const headers = [
      ...
      t('usage.kiroCredits'), t('usage.kiroListPriceCostEstimate'), t('usage.kiroSavingsCostEstimate'), t('usage.kiroDiscountRateEstimate'), t('usage.kiroInputTokens'), t('usage.kiroOutputTokens'),
    ...
    while (true) {
      ...
      const rows = (res.items || []).map((log: AdminUsageLog) => [
        ...
        log.kiro_discount_rate_estimate != null ? `${(log.kiro_discount_rate_estimate * 100).toFixed(1)}%` : '',
        log.upstream_kiro_input_tokens ?? '', log.upstream_kiro_output_tokens ?? '',
