...
// openAIStreamProviderErrorFields 从 response.failed / error 事件里抽出上游结构化的
// error.code / error.type。fork 独有：官方基线只留 Message，classifyError 只能靠文案匹配。
	...
	for documentScanner.Scan() {
		...
		if data, ok := extractOpenAISSEDataLine(line); ok {
			...
			if eventType == "response.failed" {
				...
				if !cyberHit {
					// 既没走 cyber 也没切号/透传：failed 事件原样转给客户端，
					// 但仍要落 ops 上游错误事件，否则「输出已开始后失败」在监控里不可见。
