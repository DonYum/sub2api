...
func openAIStreamFailedEventProviderFields(payload []byte, message string) (code string, errType string) {
	...
	switch {
	case isOpenAIContextWindowError(message, payload):
		if code == "" {
			code = "context_length_exceeded"
		}
		if errType == "" || strings.Contains(errType, "service_unavailable") {
...
func (s *OpenAIGatewayService) recordOpenAIStreamUpstreamError(
	...
	statusCode := openAIStreamFailureStatus(payload, message)
	providerCode, providerType := openAIStreamFailedEventProviderFields(payload, message)
	detail := ""
