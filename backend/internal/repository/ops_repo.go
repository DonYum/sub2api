...
func buildOpsErrorLogsWhere(filter *service.OpsErrorLogFilter) (string, []any) {
	...
	// into recovered provider-health rows (upstream/account_auth), or when the
	// caller asks for the unfiltered ops monitoring view. Request-error endpoints
	// never set the opt-in and retain this guard.
...
func opsFilterIncludesRecoveredProviderRows(filter *service.OpsErrorLogFilter, phaseFilter string) bool {
	...
	if len(filter.ErrorPhasesAny) == 0 {
		// No phase constraint at all: this is the general ops monitoring view,
		// which must show recovered provider failures (e.g. OpenAI capacity
		// errors that a later retry recovered) next to client-visible errors.
		return true
	}
