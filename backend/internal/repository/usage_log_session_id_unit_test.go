...
// is followed by the coding-agent terminal fields; created_at is always last.
func TestPrepareUsageLogInsert_SessionIDArgWiring(t *testing.T) {
	require.Len(t, usageLogInsertArgTypes, 70, "arg-type table must include session_id")
	...
	// created_at is last; session_id is followed by terminal-observability fields.
	sessionArg := prepared.args[len(prepared.args)-13]
	...
	require.Equal(t, "text", usageLogInsertArgTypes[len(usageLogInsertArgTypes)-13],
...
func TestPrepareUsageLogInsert_SessionIDNullWhenAbsent(t *testing.T) {
	...
	nsEmpty := preparedEmpty.args[len(preparedEmpty.args)-13].(sql.NullString)
