...
func scanUsageLog(scanner interface{ Scan(...any) error }) (*service.UsageLog, error) {
	var (
		...
		clientMachineID           sql.NullString
		clientMachineSource       sql.NullString
		clientDeviceID            sql.NullString
		clientAccountUUID         sql.NullString
		clientOriginator          sql.NullString
		codexInstallationID       sql.NullString
		codexWindowID             sql.NullString
		codexSessionID            sql.NullString
		codexThreadID             sql.NullString
		codexTurnID               sql.NullString
		terminalHash              sql.NullString
		createdAt                 time.Time
	)
	...
	if clientMachineID.Valid {
		log.ClientMachineID = &clientMachineID.String
	}
	if clientMachineSource.Valid {
		log.ClientMachineSource = &clientMachineSource.String
	}
	if clientDeviceID.Valid {
		log.ClientDeviceID = &clientDeviceID.String
	}
	if clientAccountUUID.Valid {
		log.ClientAccountUUID = &clientAccountUUID.String
	}
	if clientOriginator.Valid {
		log.ClientOriginator = &clientOriginator.String
	}
	if codexInstallationID.Valid {
		log.CodexInstallationID = &codexInstallationID.String
	}
	if codexWindowID.Valid {
		log.CodexWindowID = &codexWindowID.String
	}
	if codexSessionID.Valid {
		log.CodexSessionID = &codexSessionID.String
	}
	if codexThreadID.Valid {
		log.CodexThreadID = &codexThreadID.String
	}
	if codexTurnID.Valid {
		log.CodexTurnID = &codexTurnID.String
	}
	if terminalHash.Valid {
		log.TerminalHash = &terminalHash.String
	}
