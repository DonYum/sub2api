...
#### OpenAI Responses 首输出超时与换号策略

`gateway.openai_first_output_timeout` 限制 OpenAI Responses 流等待首个模型输出的时间，单位为秒。默认值为 `300`；设置为 `0` 表示禁用，非零值必须在 `30-1800` 之间。对客户端等待上限短于默认值的部署，建议显式配置较短的保险丝，例如：
...
该配置用于把长时间无首输出的请求转换为可观测、可重试的网关失败，避免客户端先断开后，上游仍继续处理并在服务端留下误导性的成功记录。历史生产事件中，客户端约在 `221s` 主动断开，而上游约在 `250s` 才完成；旧的 `300s` 默认超时晚于客户端耐心窗口，因此无法提前终止请求。将保险丝设为 `180s` 后，应持续观察首输出超时、客户端取消、最终 5xx 与 TTFT 分布，确认没有误伤正常的慢推理请求。一次上线后的约 `15h` 观察窗中，`180s` 首输出超时为 `0` 次，历史约 `220s` 断流也未复现；因此当前结论是保留保险丝，但不为尚未出现的超时增加换号逻辑。

首输出超时当前**不会自动切换账号重试**。这是有意的安全边界：Responses continuation 可能携带 `previous_response_id`，客户端也可能已经收到部分输出；盲目换号会破坏会话绑定，并可能造成重复输出或重复计费。只有同时满足以下条件时，才应重新评估自动 failover：

- 已经出现可重复的 `first_output_timeout`，而不是客户端取消或流中数据超时；
- 尚未向客户端发送任何模型输出；
- 请求不含 `previous_response_id`，不是 continuation；
- 重放具有明确的幂等与计费去重保证；
- 监控能够区分原账号失败、换号结果与最终客户端状态。

如果观察窗口内没有首输出超时，且历史断流未复现，建议保持当前保险丝，不为尚未发生的问题修改调度。等监控出现真实样本后，再根据失败频率、TTFT 长尾和账号池状态决定是否实现或引入上游的 failover 改动。
